...
namespace Proyect_InvOperativa.Services
{
    public class VentasService
    {
        ...
        #region Traer Todas las ventas
        public async Task<List<VentasDto>> getAllVentas()
        {

            var listaDtos = new List<VentasDto>();
            var allVentas = await _ventasRepository.GetAllAsync();
            foreach (var venta in allVentas)
            {
                var detallesVentasByID = await _ventasRepository.GetDetalleVentaByVenta(venta.nVenta);
                if (detallesVentasByID != null)
                {
                    listaDtos.Add(detallesVentasByID);
                }
            }
            return listaDtos;
